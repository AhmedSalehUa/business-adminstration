...
public class TrackIn {
    ...
    int userId;
    String user;
    ObservableList<TrackInDetails> details;

    public TrackIn() {
    }

    public TrackIn(int id, int lineId, String date, String user) {
        this.id = id;
        this.lineId = lineId;
        this.date = date;
        this.user = user;
    ...
    public ObservableList<TrackInDetails> getDetails() {
        return details;
    }

    public void setDetails(ObservableList<TrackInDetails> details) {
    ...
    public boolean Add() throws Exception {
        int i = 1;
        PreparedStatement ps = db.get.Prepare("INSERT INTO `st_per_line_in`(`line_id`, `date`, `user_id`) VALUES (?,?,?)");
        ps.setInt(i++, lineId);
        ps.setString(i++, date);
        ps.setInt(i++, userId);
        ps.execute();
    ...
    public boolean Edite() throws Exception {
        DeleteDetails();
        int i = 1;
        PreparedStatement ps = db.get.Prepare("UPDATE `st_per_line_in` SET `line_id`=?,`date`=?,`user_id`=? WHERE `id`=?");
        ps.setInt(i++, lineId);
        ps.setString(i++, date);
        ps.setInt(i++, userId);
        ps.setInt(i++, id);
    ...
    public boolean Delete() throws Exception {
        DeleteDetails();
        PreparedStatement ps = db.get.Prepare("DELETE FROM `st_per_line_in` WHERE `id`=?");
        ps.setInt(1, id);
        ps.execute();
        Logs.Add(ps.toString());
        return true;
...
//
    ...
    public boolean AddDetails() throws Exception {
        PreparedStatement ps = db.get.Prepare("INSERT INTO `st_per_line_in_details`( `line_out_id`, `store_product_id`,`product_id`,`amount`, `cost`) VALUES (?,?,?,?,?)");

        for (TrackInDetails a : details) {
            ps.setInt(1, id);
    ...
    public boolean DeleteDetails() throws Exception {
        PreparedStatement ps = db.get.Prepare("DELETE FROM `st_per_line_in_details` WHERE `line_out_id`=?");
        ps.setInt(1, id);
        ps.execute();
        Logs.Add(ps.toString());
        return true;
...
//
    ...
    public static String getAutoNum() throws Exception {
        return db.get.getTableData("SELECT IFNULL(MAX(`id`)+1,1) FROM `st_per_line_in`").getValueAt(0, 0).toString();
    }
}
