...
public class TrackInDetails {
    ...
    public TrackInDetails(int id, String product, String amount, String cost, String totalcost) {
        this.id = id;
    ...
    public TrackInDetails(int id, ObservableList<StoreProducts> data, String selectedpro, String amount, String cost, String totalCostString) {
        this.id = id;
        this.productsCombo = new ComboBox(data);
        this.amountField = new TextField(amount);
        this.costField = new TextField(cost);
        this.totalcost = Double.toString(Double.parseDouble(cost) * Double.parseDouble(amount));
    ...
    public static ObservableList<TrackInDetails> getData(int id,ObservableList<StoreProducts> data1) throws Exception {
        ObservableList<TrackInDetails> data = FXCollections.observableArrayList(); 
        ResultSet rs = db.get.getReportCon().createStatement().executeQuery("SELECT `sl_out_line_products`.`id`,`st_products`.`name`, `sl_out_line_products`.`amount`, `sl_out_line_products`.`cost`  FROM `sl_out_line_products`,`st_products` WHERE `st_products`.`id` =`sl_out_line_products`.`product_id` AND `sl_out_line_products`.`line_id`='" + id + "'");
        while (rs.next()) {
            data.add(new TrackInDetails(rs.getInt(1), data1, rs.getString(2), rs.getString(3), rs.getString(4),""));
        }
        return data;
    }
