 ...
 * @author AHMED
 */
public class ProviderInvoiceReturnEditsController implements Initializable {
    ...
    @FXML
    private ComboBox<ProviderReturned> invoiceId;
    @FXML
    ...
    private void fillCombo() {
        progress.setVisible(true);
        Service<Void> service = new Service<Void>() {
            ObservableList<ProviderReturned> data;
            ...
            protected void succeeded() {
                ...
                invoiceId.setConverter(new StringConverter<ProviderReturned>() {
                    @Override
                    public String toString(ProviderReturned object) {
                        return Integer.toString(object.getId());
                    }

                    @Override
                    public ProviderReturned fromString(String string) {
                        return null;
                    }
                });
                invoiceId.setCellFactory(cell -> new ListCell<ProviderReturned>() {
                    ...
                    @Override
                    protected void updateItem(ProviderReturned person, boolean empty) {
                            ...
                            public void run() {
                                try {
                                    ...
                                    if (result.get() == ButtonType.OK) {
                                        ProviderReturned in = new ProviderReturned();
    ...
    private void getData(int id) {
        try {
            ProviderReturned in = ProviderReturned.getDataById(id).get(0);
            ...
            invoiceTable.setItems(ProviderReturnedDetails.getData(id, invoiceId.getSelectionModel().getSelectedItem().getInvoiceId()));
            invoiceTable.getItems().add(new ProviderReturnedDetails(1, data, "0", "0", "0", null));
            invoiceTable.setOnKeyReleased((event) -> {
                ...
                if (event.getCode() == KeyCode.ENTER && invoiceTable.getSelectionModel().getSelectedItem().getProductsCombo().getSelectionModel().getSelectedIndex() != -1
                        ...
                        && !invoiceTable.getSelectionModel().getSelectedItem().getCostField().getText().equals("0")) {
                    setTotal();
                    invoiceTable.getItems().add(new ProviderReturnedDetails(invoiceTable.getItems().size() + 1, data, "0", "0", "0", null));
