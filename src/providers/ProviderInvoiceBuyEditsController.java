 ...
 * @author AHMED
 */
public class ProviderInvoiceBuyEditsController implements Initializable {
...
//            }
    ...
    private void fillCombo() {
        progress.setVisible(true);
        Service<Void> service = new Service<Void>() {
            ObservableList<InvoiceBuy> invoices;
    ...
    private void EditeCond(ActionEvent event) {
        ...
        of.setInvoiceId(invoiceId.getSelectionModel().getSelectedItem().getId());
        int i = CondTab.getSelectionModel().getSelectedIndex();
        CondTab.getItems().remove(i);
    ...
    private void AddCond(ActionEvent event) {
        ...
        a.setInvoiceId(invoiceId.getSelectionModel().getSelectedItem().getId());
    ...
    private void getData(int id) {
        try {
            InvoiceBuy in = InvoiceBuy.getDataById(id).get(0);
            ...
            CondTab.setItems(InvoiceBuyExpenses.getData(id));
        } catch (Exception ex) {
            AlertDialogs.showErrors(ex);
        }
    }
